const passwords = {
    "4211": {
        ...
        loans: [
            {
                planDate: "24-05-2025",
                endDate: "-- <p style='color: #ff6200ff;'>(Interest adding stopped by #PY@0212)</p>",
                interest: 0,
                takenAmount: 25000,
                takenFrom: "MLending",
                fineRate: 5
...
" src="https://static-cdn.jtvnw.net/badges/v1/3267646d-33f0-4b17-b3df-f923a41db1d0/2" alt="">
    ...
    filter: invert(1);display: none;" src="https://static-cdn.jtvnw.net/emoticons/v2/emotesv2_7d7473ef8ba54ce2b2f8e29d078f90bf/default/dark/2.0" alt="">
        ...
        // Render special content (text or image)
        ...
        try {
            if (user.showCustomContent === "yes" && user.customContent && user.customContent.value) {
                ...
                console.log(`Rendering specialContent for user ${userInput}: type=${user.customContent.type}, value=${user.customContent.value}, url=${contentUrl}`);
                const contentHtml = user.customContent.type === "text"
                    ...
                    : `
                        <h3 style="color: white; font-size: 16px;">Special Offer</h3>
                        <img class="applybtn" src="${user.customContent.value}" alt="Special Offer" style="cursor: pointer;" onerror="this.src=''; console.log('Image failed to load: ${user.customContent.value}');">
                    `;
                if (contentUrl) {
                    specialContentDiv.innerHTML = `
                        <a href="${contentUrl}" style="text-decoration: none; display: block;" aria-label="Special offer for ${user.name}">
                            ${contentHtml}
                        </a>
                    `;
                    ...
                    console.log(`No valid URL for user ${userInput}, rendering content without link`);
                }
            } else {
                console.log(`No special content for user ${userInput}: showCustomContent=${user.showCustomContent}, customContent=${JSON.stringify(user.customContent)}`);
                specialContentDiv.innerHTML = "";
            }
        } catch (error) {
...
;
    ...
    margin-top: 10px;
    font-weight: 400;
    letter-spacing: 0.2px;" onclick="downloadSingleLoan(${index})">
